...
> **固定视频入口：**[4 分 10 秒中文字幕 Demo](https://gikemj.github.io/tightening-quality-guardian/video/quality-guardian-v2.mp4)。视频与在线页面使用同一组 ST-FAS-07 / TQ-17 / P03 口径。
...
### 6.2 4 分 10 秒 Demo 视频脚本

| 时间 | 画面 | 讲解重点 |
|---:|---|---|
| 0:00–0:24 | 项目范围、工位对象和数据边界 | 先把一个工位、一个工具、一个紧固点讲清楚 |
| 0:24–0:52 | 基线窗口与最近窗口 | 同车型/同工位/同程序，避免混口径比较 |
| 0:52–1:20 | 多信号变化 | 规格内 1.56σ 漂移、角度离散和重试同步变化 |
| 1:20–1:48 | 工位工程示意 | ST-FAS-07、TQ-17、P03 如何进入同一证据链 |
| 1:48–2:14 | 证据关系与候选原因 | 事实、候选、待补证分层，不把历史类比写成根因 |
| 2:14–2:42 | 验证动作草案 | 责任角色、时间提示、验收字段和证据编号 |
| 2:42–3:08 | 管理员工作台与状态机 | 审批后才建任务，验证不足不能结案 |
| 3:08–3:40 | 服务端受控模型与边界 | 最小案卷、固定 JSON、密钥隔离、确定性回退 |
| 3:40–4:10 | 回到工位、试点与固定入口 | 先做深，再用企业数据校准和复核收益 |

视频固定入口已在 GitHub Pages 发布，提交前仍应使用无痕窗口和手机网络各抽检一次。
